    ...
    static MoveRepresentation::Move* generateKingMoves(int8_t kingPos, board::Board gameBoard, bool isWhite, MoveRepresentation::Move* toPutMoves){
        ...
        for(short adjacent : adjacents){
            ...
            if(targetVal != board::INVALID){
                ...
                if(isWhite && (targetVal < board::WP || targetVal > board::WK)){
                    ...
                    toPutMoves[index] = MoveRepresentation::Move(kingPos, kingPos + adjacent, 0u, 0u, (targetVal != board::EMPTY)? targetVal : board::EMPTY, board::EMPTY, board::WK);
                    index++;
                }
                //black, check for ally piece, can't move onto them
                    ...
                    //TODO: Note that if we go ahead with making EMPTY the "capturedPiece" default (no capture), then this branch (target...)? x : y is unnecessary
                    //TODO: check enPassant and promote number
                    toPutMoves[index] = MoveRepresentation::Move(kingPos, kingPos + adjacent, 0u, 0u, (targetVal != board::EMPTY)? targetVal : board::EMPTY, board::EMPTY, board::BK);
                    index++;
                }
            }
        }
        ...
        if(isWhite){
            ...
            if(gameBoard.CWQ && gameBoard.chessboard[92] == board::EMPTY && gameBoard.chessboard[93] == board::EMPTY && gameBoard.chessboard[94] == board::EMPTY){
                ...
                //TODO: check enPassant and promote number
                toPutMoves[index] = MoveRepresentation::Move(kingPos, 93, 1u, 0u, board::EMPTY, board::EMPTY, board::WK);
                index++;
            }
            ...
            if(gameBoard.CWK && gameBoard.chessboard[96] == board::EMPTY && gameBoard.chessboard[97] == board::EMPTY){
                //TODO: are we doing 64 or 120 for spot indexes?
                //TODO: check enPassant and promote number
                toPutMoves[index] = MoveRepresentation::Move(kingPos, 97, 2u, 0u, board::EMPTY, board::EMPTY, board::WK);
//                index++;
            }
        }
        //black
            ...
            if(gameBoard.CBQ && gameBoard.chessboard[22] == board::EMPTY && gameBoard.chessboard[23] == board::EMPTY && gameBoard.chessboard[24] == board::EMPTY){
                //TODO: are we doing 64 or 120 for spot indexes?
                //TODO: check enPassant and promote number
                toPutMoves[index] = MoveRepresentation::Move(kingPos, 23, 1u, 0u, board::EMPTY, board::EMPTY, board::BK);
                index++;
            }
            //right (king) side castle
            if(gameBoard.CBK && gameBoard.chessboard[26] == board::EMPTY && gameBoard.chessboard[27] == board::EMPTY){
                //TODO: are we doing 64 or 120 for spot indexes?
                //TODO: check enPassant and promote number
                toPutMoves[index] = MoveRepresentation::Move(kingPos, 27, 2u, 0u, board::EMPTY, board::EMPTY, board::BK);
//                index++;
            }
        }
